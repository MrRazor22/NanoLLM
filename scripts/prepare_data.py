...
def build_synthetic_samples(rng: random.Random, count: int) -> List[Dict]:
    ...
    for _ in range(count):
        ...
        ins_choice = rng.choice(["Which category best handles this issue?", "Identify the correct classification for this event.", None])
        ins_urgent = rng.choice(["Is this an urgent or critical event?", None])
        ins_sev = rng.choice(["What is the severity level of this issue?", None])
        records.append({
            "state": text,
            "questions": [
                ["category", "choice", target_idx, options, ins_choice],
                ["is_urgent", "noul", 1.0 if urgent else 0.0, None, ins_urgent],
                ["severity", "score", round(sev + rng.uniform(-0.03, 0.03), 3), None, ins_sev]
            ]
        })
    return records

def build_dataset_samples(dataset_name: str, sub: Any, text_k: str, label_k: str, q_name: str, rng: random.Random, limit: int, instr: Optional[str] = None) -> List[Dict]:
    ...
    for row in ds:
        ...
        ins = rng.choice([instr, None]) if instr else None
        ins_urg = rng.choice(["Is this urgent?", None])
        records.append({
            "state": text,
            "questions": [
                [q_name, "choice", options.index(label), options, ins],
                ["is_urgent", "noul", urgent, None, ins_urg],
                ["severity", "score", round(rng.uniform(0.7, 0.95) if urgent else rng.uniform(0.1, 0.4), 3), None, None]
            ]
        })
...
def main():
    ...
    banking = build_dataset_samples("mteb/banking77", None, "text", "label_text", "intent", rng, 10000, "Which banking intent does this message express?")
    massive = build_dataset_samples("mteb/amazon_massive_intent", "en", "text", "label_text", "action", rng, 12000, "What is the user's intent or action?")
    sentiment = build_dataset_samples("mteb/tweet_sentiment_extraction", None, "text", "label_text", "sentiment", rng, 3000, "What is the sentiment of this text?")
